...
def generate_bash():
    with open("train0.sh", 'w') as f:
        for i in range(0, 25):
            f.write("python get_knowledge_vector.py train_dream %d &\n" % i)
        f.write('wait')
    ...
    with open("dev0.sh", 'w') as f:
        for i in range(0, 25):
            f.write("python get_knowledge_vector.py dev_dream %d &\n" % i)
        f.write('wait')
    ...
    with open("test0.sh", 'w') as f:
        for i in range(0, 25):
            f.write("python get_knowledge_vector.py test_dream %d &\n" % i)
        f.write('wait')
