...
if __name__ == "__main__":
    ...
    for entry in tqdm(batch_data, desc="bash_id: %d" % batch_id):
        ...
        for i in range(len(knowledge_list)):
            ori_triple_list += knowledge_list[i]
        # 遍历每个三元组，去除重复项
        for i in range(len(ori_triple_list)):
            ls = ori_triple_list[i].split("||")
            curr_weight = float(ls[3])
            operation_list.append(1)
            for i in range(len(sub_list)):
                if (ls[1] == sub_list[i] and ls[2] == obj_list[i]) or \
                        (ls[2] == sub_list[i] and ls[1] == obj_list[i]):
        ...
        for i in range(len(operation_list)):
            if operation_list[i] == 1:
                triple_list.append(ori_triple_list[i])
        ...
        res.append({"context": context, "question": question, "choices_list": choices_list, "answer": answer,
                    "reference": reference, "ori_question": ori_question, "knowledge_vector": knowledge_vector,
                    "entry_id": entry_id})
