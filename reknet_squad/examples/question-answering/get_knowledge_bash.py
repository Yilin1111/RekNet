...
def generate_bash():
    with open("train0.sh", 'w') as f:
        for i in range(0, 25):
            f.write("python get_knowledge_squad.py train_squad %d &\n" % i)
        f.write('wait')
    ...
    with open("dev0.sh", 'w') as f:
        for i in range(0, 25):
            f.write("python get_knowledge_squad.py dev_squad %d &\n" % i)
        f.write('wait')
    ...
    with open("test0.sh", 'w') as f:
        for i in range(0, 25):
            f.write("python get_knowledge_squad.py test_squad %d &\n" % i)
        f.write('wait')
