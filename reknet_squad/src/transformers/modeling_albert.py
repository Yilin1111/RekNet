...
class AlbertForQuestionAnswering(AlbertPreTrainedModel):
    ...
    def forward(
        ...
        if knowledge_vector is not None:
            weight = knowledge_vector[:, :, 3, 0]
            # facts: 4D, 3*n(bash), 5(fact numbers), 3(timesteps), 100(dim), n is the number of questions
            facts = knowledge_vector[:, :, :3, :]
            knowledge_output, _ = self.GRU(facts.view(-1, facts.size(-2), facts.size(-1)))
            # knowledge_output: 4D, n(bash), 5(fact numbers), 3(timesteps_out), 768(dim)
            knowledge_output = knowledge_output.view(-1, facts.size(-3), knowledge_output.size(-2),
                                                     knowledge_output.size(-1))
            # knowledge_output: 3D, n(bash), 5(fact numbers), 768(dim)
            knowledge_output = knowledge_output[:, :, -1, :]
            # facts_output: 3D, n(bash), 512(seq_len), 768(dim), 这里每个知识对每个token的影响不同
            facts_output = self.sc_attention(sequence_output, knowledge_output, weight)
            # facts_output: 3D, n(bash), 512(seq_len), 768(dim), 这里每个知识对每个token的影响相同
            # facts_output = torch.mean(facts_output, 1).unsqueeze(1).repeat(1, sequence_output.shape[1], 1)
