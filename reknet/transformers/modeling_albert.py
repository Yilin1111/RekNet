...
class AlbertForMultipleChoiceSC(AlbertPreTrainedModel):
    def __init__(self, config):
        super(AlbertForMultipleChoiceSC, self).__init__(config)
        self.albert = AlbertModel(config)
        ...
        #self.GRU = nn.Linear(100, config.hidden_size)
        self.sc_attention = SCAttention(config.hidden_size, config.hidden_size)

    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
                inputs_embeds=None, labels=None, ref_input_ids=None, ref_attention_mask=None, ref_token_type_ids=None,
                knowledge_vectors=None, is_full=False):
        if not hasattr(self, '_flattened'):
            self.GRU.flatten_parameters()
        setattr(self, '_flattened', True)
        num_choices = input_ids.shape[1]
        flat_input_ids = input_ids.view(-1, input_ids.size(-1))
        flat_knowledge_vectors = knowledge_vectors.reshape(-1, knowledge_vectors.size(-3), knowledge_vectors.size(-2),
        ...
        if is_full is True:
            ref_flat_input_ids = ref_input_ids.view(-1, ref_input_ids.size(-1))
            ref_flat_token_type_ids = ref_token_type_ids.view(-1, ref_token_type_ids.size(
                -1)) if ref_token_type_ids is not None else None
            ref_flat_attention_mask = ref_attention_mask.view(-1, ref_attention_mask.size(
                -1)) if ref_attention_mask is not None else None
            ref_outputs = self.albert(
                input_ids=ref_flat_input_ids,
                attention_mask=ref_flat_attention_mask,
                token_type_ids=ref_flat_token_type_ids,
                position_ids=flat_position_ids,
                head_mask=flat_head_mask,
                inputs_embeds=flat_inputs_embeds
            )
            ref_pooled_output = ref_outputs[1]
            pooled_output = self.dense(torch.cat((pooled_output, ref_pooled_output), -1))

        pooled_output = self.dropout(pooled_output)
        weight = flat_knowledge_vectors[:, :, 3, 0]
        # facts: 4D, 3*n(bash), 15(fact numbers), 3(timesteps), 100(dim), n is the number of questions
        facts = flat_knowledge_vectors[:, :, :3, :]
        ...
        facts_output = self.sc_attention(ref_outputs[0], knowledge_output, weight)
        facts_output = torch.mean(facts_output, 1).squeeze(1)
        pooled_output = self.dense(torch.cat((pooled_output, facts_output), -1))

        # ori_logits = self.classifier(outputs[1])
        # reshaped_ori_logits = ori_logits.view(-1, num_choices)
        logits = self.classifier(pooled_output)
        ...
        if labels is not None:
            loss_fct = CrossEntropyLoss()
            # ori_loss = loss_fct(reshaped_ori_logits, labels)
...
class AlbertForMultipleChoiceTRM(AlbertPreTrainedModel):
    def __init__(self, config):
        super(AlbertForMultipleChoiceTRM, self).__init__(config)
        self.albert = AlbertModel(config)
        self.classifier = nn.Linear(config.hidden_size, 1)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.init_weights()
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
        ...
        knowledge_output = knowledge_output[:, :, -1, :]
        # output: 2D, 3*n(bash), 768(dim)
        facts_output = self.attention(knowledge_output, ref_outputs[0], weight)
        facts_output = torch.mean(facts_output, 1).squeeze(1)
        pooled_output = self.dense(torch.cat((pooled_output, facts_output), -1))
